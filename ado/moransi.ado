...
program moransi, sortpreserve rclass
	...
	/*+++++CALL Mata Program+++++*/
	mata: calcmoransi("`vY'", "`lon'", "`lat'", `fmdms', "`swmtype'", `dist', "`unit'", `dd', "`wvar'", `sweight', `appdist', `dispdetail', `matsave', `genvar', "`touse'")
	/*+++++END Mata Program+++++*/
	...
	if( `genvar' == 1 ){
		...
		qui: replace lmoran_cat_`vY'_`swmtype_short' = 1 if splag_`vY'_`swmtype_short' >= `mean_wy' & `vY' >= `mean_y'
		qui: replace lmoran_cat_`vY'_`swmtype_short' = 2 if splag_`vY'_`swmtype_short' < `mean_wy' & `vY' >= `mean_y'
		qui: replace lmoran_cat_`vY'_`swmtype_short' = 3 if splag_`vY'_`swmtype_short' >= `mean_wy' & `vY' < `mean_y'
		qui: replace lmoran_cat_`vY'_`swmtype_short' = 4 if splag_`vY'_`swmtype_short' < `mean_wy' & `vY' < `mean_y'
		...
		/*Graph*/
		if( "`graph'" != "" | "`gsaving'" != "" ){
			twoway (scatter splag_`vY'_`swmtype_short' `vY', ms(oh) mc(navy) yaxis(1 2) xaxis(1 2)) ///
				(lfit splag_`vY'_`swmtype_short' `vY', lw(medthick)) ///
				, ///
				ytitle("Spatial Lag of `vY'", tstyle(size(medium)) axis(1)) ///
				xtitle("`vY'", tstyle(size(medium)) height(6) axis(1)) ///
				ytitle("", axis(2)) ///
				xtitle("", axis(2)) ///
				ylabel(, ang(h) labsize(medium) format(%2.0f) nogrid axis(1)) ///
				xlabel(, labsize(medium) format(%2.0f) nogrid axis(1)) ///
				ylabel(, ang(h) labsize(medium) format(%2.0f) nogrid axis(2)) ///
				xlabel(, labsize(medium) format(%2.0f) nogrid axis(2)) ///
...
void calcmoransi(vY, lon, lat, fmdms, swmtype, dist, unit, dd, wvar, sweight, appdist, dispdetail, matsave, genvar, touse)
{
	...
	for( i = 1; i <= cN; ++i ){
		...
		/*Distance = 0 for Missing between i and i */
		if(missing(vDist) > 1){
		...
		if( swmtype == "exp" ){
			if( sweight == 0 ){
				vW = ( vDist :< dist ) :* exp( - dd :* mW ) 
			}
			else if( sweight == 1 ){
				vW = ( vDist :< dist ) :* exp( - dd :* mW ) :* (vZ') 
			}
			vDist = .
			/*ERROR CHECK*/
